  ...
  methods: {
    async ProductById() {
      try {
        const {
          data
        } = await catalogApi["a" /* default */].getProductByIdCategory(this.$axios, this.$route.params.id);
        this.listProduct = data.product;
        console.log(data); // data lay json tu server gom success message productdetail T .productDetail laf lay rieng detail thoi
