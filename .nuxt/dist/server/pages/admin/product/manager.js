...
//
//
/* harmony default export */ var managervue_type_script_lang_js_ = ({layout:'admin',middleware:'authAdmin',data(){return{listManager:[]};},async created(){await this.Managerlist();},methods:{async Managerlist(){try{const{data}=await catalogApi["a" /* default */].getProducts(this.$axios);this.listManager=data.products;}catch(err){console.log(err);}},OnDelete(id){const swalWithBootstrapButtons=external_sweetalert2_default.a.mixin({customClass:{confirmButton:'btn btn-success',cancelButton:'btn btn-danger'},buttonsStyling:false});swalWithBootstrapButtons.fire({title:'Bạn chắn chắn muốn xóa',text:'Dữ liệu không thể khôi phục!',icon:'warning',showCancelButton:true,confirmButtonText:'Xóa luôn!',cancelButtonText:'Thôi em rút lui!',reverseButtons:true}).then(result=>{if(result.isConfirmed){catalogApi["a" /* default */].deleteProduct(this.$axios,id);swalWithBootstrapButtons.fire('Đã xóa thành công!','Sản phẩm đã được tiễn về ông bà!','success');this.$router.push('admin/product/manager');this.$router.go();}else if(/* Read more about handling dismissals below */result.dismiss===external_sweetalert2_default.a.DismissReason.cancel){swalWithBootstrapButtons.fire('Thôi không xóa nữa','Sản phẩm của bạn còn ở đấy','error');}});}}});
