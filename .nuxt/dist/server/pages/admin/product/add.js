...
//
//
/* harmony default export */ var addvue_type_script_lang_js_ = ({layout:'admin',data(){return{dataSubmit:{Name:'',Game_genre:'',Decription:'',Video_Game:'',Free_Space:null,Time_Update:new Date().toISOString().substr(0,10),Image_1:'',Image_2:'',Image_3:'',Point_Rare:'',Ram:null,OS:'',Languages:'',Player:'',Link_Fshare:'',Link_Vip:'',ID_Category:1},categories:[]};},async created(){await this.getCategories();},methods:{async getCategories(){try{const{data}=await catalogApi["a" /* default */].getCategories(this.$axios);this.categories=data;}catch(err){console.log(err);}},async onCreatePost(){const data={Name:this.dataSubmit.Name,Game_genre:this.dataSubmit.Game_genre,Decription:this.dataSubmit.Decription,Video_Game:this.dataSubmit.Video_Game,Free_Space:this.dataSubmit.Free_Space,Time_Update:this.dataSubmit.Time_Update,Image_1:this.dataSubmit.Image_1,Image_2:this.dataSubmit.Image_2,Image_3:this.dataSubmit.Image_3,Point_Rare:this.dataSubmit.Point_Rare,Ram:this.dataSubmit.Ram,OS:this.dataSubmit.OS,Languages:this.dataSubmit.Languages,Player:this.dataSubmit.Player,Link_Fshare:this.dataSubmit.Link_Fshare,Link_Vip:this.dataSubmit.Link_Vip,ID_Category:this.dataSubmit.ID_Category};try{await this.$axios.$post('/product',data);// console.log('Signup successfully!')
const Toast=external_sweetalert2_default.a.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000,timerProgressBar:true,didOpen:toast=>{toast.addEventListener('mouseenter',external_sweetalert2_default.a.stopTimer);toast.addEventListener('mouseleave',external_sweetalert2_default.a.resumeTimer);}});Toast.fire({icon:'success',title:'Added product successfully'});console.log(this.$router);this.$router.push('/admin');}catch(error){console.log(error);}}}});
