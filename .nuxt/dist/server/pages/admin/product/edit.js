...
//
//
/* harmony default export */ var editvue_type_script_lang_js_ = ({layout:'admin',data(){return{listProducts:[],listCategory:[]};},async created(){await this.getAllProducts();await this.getCategory();},methods:{async getAllProducts(){try{const{data}=await catalogApi["a" /* default */].getByIdProducts(this.$axios,this.$route.params.id);this.listProducts=data.productDetail;console.log(this.listProducts);}catch(err){console.log(err);}},async getCategory(){try{const{data}=await catalogApi["a" /* default */].getCategories(this.$axios);this.listCategory=data;}catch(err){console.log(err);}},OnEdit(){external_sweetalert2_default.a.fire({title:'Bạn có muốn lưu lại tất cả dữ liệu vừa nhập?',showDenyButton:true,showCancelButton:true,confirmButtonText:'Lưu đi',denyButtonText:`Đổi ý rồi! Không lưu nữa`}).then(result=>{/* Read more about isConfirmed, isDenied below */if(result.isConfirmed){this.$axios.$put('/product/update/',this.listProducts);external_sweetalert2_default.a.fire('Lưu xong rồi nè!','','success');this.$router.push('/admin/product/manager');}else if(result.isDenied){external_sweetalert2_default.a.fire('Hờn rồi! Không lưu nữa','','info');this.$router.push('/admin/product/manager');}});}}});
