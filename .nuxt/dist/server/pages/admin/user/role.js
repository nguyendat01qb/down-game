  ...
  methods: {
    async Managerlist() {
      try {
        const {
          data
        } = await catalogApi["a" /* default */].getAllEmailAd(this.$axios);
    ...
    OnSave(id) {
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
          ...
          swalWithBootstrapButtons.fire('Đã lưu thành công!', 'Account đã được #/𝄞 chức!', 'success');
          this.$router.push('admin/user/role');
          this.$router.go();
        } else if (
        /* Read more about handling dismissals below */
        result.dismiss === external_sweetalert2_default.a.DismissReason.cancel) {
          swalWithBootstrapButtons.fire('Thôi không lưu nữa', 'Hờn luôn rồi!', 'error');
        }
      });
    }
