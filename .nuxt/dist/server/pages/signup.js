...
//
//
/* harmony default export */ var signupvue_type_script_lang_js_ = ({middleware:"auth",auth:"guest",data(){return{form:{email:null,phone:null,password:null,re_password:null,Date:new Date().toISOString().substr(0,10)},listEmail:[]};},validations:{form:{email:{required: validators_["required"],emailValidator:validators_["email"]},password:{required: validators_["required"],minLength:Object(validators_["minLength"])(6)},phone:{required: validators_["required"],maxLength:Object(validators_["maxLength"])(10)},re_password:{sameAs:Object(validators_["sameAs"])('password')}}},async created(){await this.AllListEmail();},computed:{isFormValid(){return!this.$v.form.$invalid;}},methods:{async AllListEmail(){try{const{data}=await catalogApi["a" /* default */].getAllEmail(this.$axios);this.listEmail=data.accs;console.log(this.listEmail);}catch(err){console.log(err);}},// eslint-disable-next-line require-await
async onClickEmail(){if(this.listEmail.length===this.form.email){console.log("Email da ton tai");}},async register(){const data={Email:this.form.email,Phone:this.form.phone,Password:this.form.password,CreateAt:this.form.Date};try{const res=await this.$axios.$post('/account/signup',data);console.log(res);// console.log('Signup successfully!')
const Toast=external_sweetalert2_default.a.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000,timerProgressBar:true,didOpen:toast=>{toast.addEventListener('mouseenter',external_sweetalert2_default.a.stopTimer);toast.addEventListener('mouseleave',external_sweetalert2_default.a.resumeTimer);}});Toast.fire({icon:'success',title:'Signed up successfully'});console.log(this.$router);this.$router.push('/signin');}catch(error){console.log(error);}}}});
